...
<div>
   ...
   <a class="chat-button" ><img @click="toggleChatBot()" src="../../../../public/images/chatbot-svgrepo-com.svg" class="img"></a>
   <div id="chatbot" class="chatbot">
      ...
      <div v-for="message in messages" :key="message.id" class=" message d-flex " >
          ...
          <div  v-if="(message.id==5 || message.id==6 || message.id==7 ) && message.isBot==true" class="d-flex"> 
              ...
              <p  v-if=" message.id!=7 " class=" bot-message " @click.once="getUserAnswer($event)"  style="margin-left: 29px;">{{  message.content }}</p>
              <p  v-else class=" bot-message " @click.once="getUserAnswer($event)"  >{{  message.content }}</p> 
          </div>
